...
public class DtxnInitiatorMailbox implements Mailbox
{
	...
	public void removeSite(int siteId)
	{
		...
		for (InFlightTxnState state : m_pendingTxns.values())
		{
			...
			if (!state.siteIsCoordinator(siteId)) {
				continue;
			}
	...
	public void deliver(VoltMessage message) {
		...
		synchronized (m_initiator) {
			...
			assert((message instanceof InitiateResponseMessage) || (message instanceof PhysicalLogResponseMessage));
			...
			if (message instanceof InitiateResponseMessage) {
				...
				final PhysicalLogResponseMessage r = (PhysicalLogResponseMessage) message;
