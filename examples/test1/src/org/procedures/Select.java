...
public class Select extends VoltProcedure {
    ...
    public final SQLStmt selectItem =
      new SQLStmt("SELECT TEST1_ID,  TEST1_ITEM " +
                  "FROM TEST1 WHERE  TEST1_ID = ?");

    public VoltTable[] run( int TEST1_ID ) throws VoltAbortException {
        // Add a SQL statement to the current execution queue
        voltQueueSQL( selectItem, TEST1_ID );
